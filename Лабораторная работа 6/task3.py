...
def to_csv_file(filename, headers, rows, delimiter = ",", new_line = "\n"):  # реализация функции to_csv_file
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"{delimiter.join(headers)}{new_line}")
        i = 0
        while i < len(rows):
            f.write(f"{delimiter.join(rows[i])}{new_line}")
            i += 1
...
to_csv_file(OUTPUT_FILE, headers_list, data)  # вызов функции to_csv_file и запись данных в файл
